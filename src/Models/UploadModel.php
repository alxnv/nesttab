 ...
 * файл .cnt_lock - содержит чилсло n в текстовом виде
...
class UploadModel {
        ...
        if ($n == 0) {
            ...
            do {
                ...
                if (!\Alxnv\Nesttab\core\FileHelper::numberOfFilesInDirLessThen(public_path() .
                        '/upload/' . $k, $n + 2)) {
                    // если в директории больше файлов чем число в upload/.cnt_lock,
    ...
    public function increaseCounter() {
        ...
        if ($fp === false) \yy::gotoErrorPage ('Error opening upload/.cnt_lock');
        if (flock($fp, LOCK_EX)) { // exclusive lock
            if (($data = fread($fp, 100)) !== false) {
                if (!ftruncate($fp, 0)) {
                    \yy::gotoErrorPage ('Error truncating upload/.cnt_lock');
                }
                fseek($fp, 0);
                $n = intval($data) + 1;
        ...
        fclose($fp);
        if ($data === false) {
            \yy::gotoErrorPage ('Error writing to upload/.cnt_lock');
        }
        return $n;
    }
    
    /**
     * Получить содержимое файла upload/.cnt_lock
    ...
    public static function getCounterFile() {
        ...
        if ($s2 === false) {
            \yy::gotoErrorPage('Error writing file upload/.cnt_lock');
        }
        return $s2;
